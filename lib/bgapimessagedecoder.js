...
function decodeIdentityAddress(payload, origin, time) {
  if(payload.length === GAP_GET_IDENTITY_ADDRESS_BYTES) {
    let content = {
        ...
        deviceIdType: (payload.readUInt8(6) === 0x00) ?
...
function decodeAdvertisementReport(payload, messageId, origin, time) {
  ...
  if((messageId === SCANNER_LEGACY_ADVERTISEMENT_REPORT_MESSAGE_ID) &&
    ...
    let transmitterIdType = (payload.readUInt8(7) === 0x00) ?
                                                Raddec.identifiers.TYPE_EUI48 :
                                                Raddec.identifiers.TYPE_RND48;
    ...
    let headerBytes = dataLength + 6;

    if(transmitterIdType !== Raddec.identifiers.TYPE_EUI48) {
      headerBytes += (1 << 14); // TxAdd = random
    }
    switch(eventFlags) {
      case 0x1: // Connectable, undirected
        break;
      case 0x5: // Connectable, directed
        headerBytes += (0x1 << 8);
        break;
      case 0x4: // Non-connectable, undirected
        headerBytes += (0x2 << 8);
        break;
      case 0x8: // Scan response
        headerBytes += (0x4 << 8);
        break;
      case 0x2: // Scannable
        headerBytes += (0x6 << 8);
    ...
    let originalPacket = headerBytes.toString(16).padStart('0', 4) +
                         payload.readUIntBE(1, 6).toString(16) + data;

    if(isCompleteData) {
      let content = { transmitterId: transmitterId,
                      transmitterIdType: transmitterIdType,
                      rssiSignature: [ { rssi: rssi } ],
                      packets: [ originalPacket ] };
